...
const PublicMyChannelVideo = () => {
    
    const { apigetVideoById, apigetAllMyVideos, myChannelVideos, userChannelProfile} = useApp();
    useEffect(() => {
        const getAllMyVideos = async () => {
          await apigetAllMyVideos(1,10, userChannelProfile._id);
        }
        getAllMyVideos()
      }, [])
...
export default PublicMyChannelVideo